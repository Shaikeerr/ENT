...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (isset($_POST['cours'])) {
        ...
        echo '<form action="db/ajout_ressource_db.php" method="POST">';
        echo '<a href="ajout_ressource.php">Retour</a>';
        echo '<h1>Ajouter une sous-ressource pour ' . $result_matiere["nom_matiere"] . '</h1>';
        ...
        $sql_professeur = "SELECT * FROM professeurs JOIN link_prof_matiere ON professeurs.id_professeur = link_prof_matiere.id_professeur WHERE link_prof_matiere.id_matiere = :id_matiere";
        ...
        if (count($result_professeur) > 0) {
            foreach ($result_professeur as $row_professeur) {
                echo "<option value='" . $row_professeur["id_professeur"] . "'>" . strtoupper($row_professeur["nom_professeur"]) . ' ' . $row_professeur["prenom_professeur"] . "</option>";
            }
        } else {
            echo "<option value=''>Aucun élément trouvé</option>";
        }
        ...
        echo '<label for="nouvelle_sous_matiere">Nom Sous-matière</label>';
        echo '<input type="text" name="nouvelle_sous_matiere" id="nouvelle_sous_matiere" placeholder="Exemple : R4.02 - Anglais">';
